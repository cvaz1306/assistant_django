...
class intResp():
    ...
    actionX=""
    def process(input):
        if(inputsCompleted==0):
            user_commands.append(input)
            actionX=""
            ...
            inputsRequired=actionIns.get(actionX)
            return resps.get(actionX)[inputsCompleted]
        if(inputsCompleted < inputsRequired and not user_commands==0):
            user_commands.append(input)
            return resps.get(actionX)[inputsCompleted]
            
        inputsCompleted=inputsCompleted+1
        if(inputsCompleted==inputsRequired):
            output=gr(inpArr=user_commands)
            inputsRequired=0
            inputsCompleted=0
            return output
